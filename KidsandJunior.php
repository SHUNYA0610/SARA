<?php include('header6.php'); ?>
<main>
  <section class="sab-visual">
  <div class="openbtn1"><span></span><span></span><span></span></div>
<?php include('nav.php'); ?>
    <div class="sab-visual-content">
      <h2 class="sab-visual-heading">Menu</h2>
    <div>
  </section>
  <div class="sabnav">
    <ul class="column2">
     <li><a href="top.php"><i class="fa-solid fa-house" style="color: #D085FD;"></i>トップ</a></li>
     <li><a href="menu.php"><i class="fa-solid fa-angle-up fa-rotate-90" style="color: #D085FD;"></i>メニュー</a></li>
     <li><a href="KidsandJunior.php"><i class="fa-solid fa-angle-up fa-rotate-90" style="color: #D085FD;"></i>キッズ&ジュニア脱毛</a></li>
    </ul>  
  </div>
  <section class=menu>
   <div class="tb-line">
      <h2>キッズ&ジュニア脱毛 人気メニュー</h2>
   </div>    
   <div class=menu-contents>
     <div class="left-line">  
        <dl>
          <db>
            <div class="menu-heading">両腕</div>
            <div class="menu-price">￥4,930円</div>
          </db>  
          <dt></dt>
        <dl>
          <db>        
            <div class="menu-heading">両足(両脚)</div>
            <div class="menu-price">￥5,380円</div>
          </db>            
     </div>  
     <div class="menu-image"><img src="./img/kidsandjunior.jpg"　alt="子供脱毛"></div>
   </div>
   <div>
   <p class="menu-text">従来の脱毛機器では、うぶ毛には対応できないことが多い状況でした。SARAで使用している脱毛機器は、キッズ&ジュニアモードが搭載されており、うぶ毛にも脱毛効果が出ております。7歳から施術できるほど優しい機器なので敏感肌の方も施術可能です。年齢が若ければ若いほど脱毛効果が長続きするので7歳〜18歳までのお子様にオススメです。</p>
   </div>    
  </section>
  <section class="price">
    <div class="price-content">
      <h2 id="price-heading">コース料金表(税込み)</h2>
      <div class="table-scroll">
      <table class="price-table">
          <tr>
            <th class="fixed01">コース</th>
            <th>時間</th>
            <th>初来店<br>（お試し）</th>
            <th>１回（都度払い）</th>
            <th>６回コース</th>
            <th>１回分<br>[約5%オフ]</th>
        　　<th>１２回コース</th>
            <th>１回分<br>[約10%オフ]</th>
          </tr>
          <tr>
            <th class="fixed01">全身脱毛+(お顔+VIO)</th>
            <td>110分</td>
            <td>￥9,240円</td>
            <td>￥18,480円</td>
            <td>￥104,800円</td>
            <td>￥17,400円</td>
            <td>￥199,800円</td>
            <td>￥16,600円</td>
          </tr>
          <tr>
            <th class="fixed01" style="background-color: #fff;">全身脱毛+(VIO)</th>
            <td>90分</td>
            <td>￥8,420円</td>
            <td>￥16,480円</td>
            <td>￥94,800円</td>
            <td>￥15,800円</td>
            <td>￥179,800円</td>
            <td>￥14,900円</td>
          </tr>
          <tr>
            <th class="fixed01">全身脱毛+(お顔)</th>
            <td>85分</td>
            <td>￥7,740円</td>
            <td>￥15,480円</td>
            <td>￥89,800円</td>
            <td>￥14,900円</td>
            <td>￥169,800円</td>
            <td>￥14,100円</td>
          </tr>
          <tr>
            <th class="fixed01" style="background-color: #fff;">全身脱毛</th>
            <td>60分</td>
            <td>￥6,740円</td>
            <td>￥13,480円</td>
            <td>￥76,800円</td>
            <td>￥12,800円</td>
            <td>￥149,800円</td>
            <td>￥12,400円</td>
          </tr>
          <tr>
            <th class="fixed01">VIO+両腕+両足</th>
            <td>50分</td>
            <td>￥5,990円</td>
            <td>￥11,980円</td>
            <td>￥69,800円</td>
            <td>￥11,600円</td>
            <td>￥129,800円</td>
            <td>￥10,800円</td>
          </tr>
          <tr>
            <th class="fixed01" style="background-color: #fff;">お顔+両腕+両足</th>
            <td>50分</td>
            <td>￥5,490円</td>
            <td>￥10,980円</td>
            <td>￥62,800円</td>
            <td>￥10,400円</td>
            <td>￥119,800円</td>
            <td>￥9,900円</td>
          </tr>
          <tr>
            <th class="fixed01">両腕+両ワキ+両足</th>
            <td>40分</td>
            <td>￥4,990円</td>
            <td>￥9,980円</td>
            <td>￥56,800円</td>
            <td>￥10,400円</td>
            <td>￥119,800円</td>
            <td>￥9,900円</td>
          </tr>
          <tr>
            <th class="fixed01" style="background-color: #fff;">VIO</th>
            <td>20分</td>
            <td>￥3,990円</td>
            <td>￥7,980円</td>
            <td>￥45,800円</td>
            <td>￥7,600円</td>
            <td>￥85,800円</td>
            <td>￥7,100円</td>
          </tr>
          <tr>
            <th class="fixed01">お顔</th>
            <td>20分</td>
            <td>￥3,490円</td>
            <td>￥6,980円</td>
            <td>￥39,800円</td>
            <td>￥6,600</td>
            <td>￥74,800円</td>
            <td>￥6,200円</td>
          </tr>
          <tr>
            <th class="fixed01" style="background-color: #fff;">うなじ・首・背中(上部)</th>
            <td>20分</td>
            <td>￥2,990円</td>
            <td>￥5,980円</td>
            <td>￥33,800円</td>
            <td>￥5,600円</td>
            <td>￥64,800円</td>
            <td>￥5,400円</td>
          </tr>
          <tr>
            <th class="fixed01">両腕<br>(ひじ・手の指・甲)</th>
            <td>15分</td>
            <td>￥1,990円</td>
            <td>￥3,980円</td>
            <td>￥22,800円</td>
            <td>￥3,800円</td>
            <td>￥42,800円</td>
            <td>￥3,500円</td>
          </tr>
          <tr>
            <th class="fixed01" style="background-color: #fff;">両足<br>(ひざ・足指・甲)</th>
            <td>20分</td>
            <td>￥2,240円</td>
            <td>￥4,480円</td>
            <td>￥25,800円</td>
            <td>￥4,300円</td>
            <td>￥47,800円</td>
            <td>￥3,900円</td>
          </tr>
          <tr>
            <th class="fixed01">両ワキ</th>
            <td>10分</td>
            <td>￥1,340円</td>
            <td>￥2,680円</td>
            <td>￥15,200円</td>
            <td>￥2,500円</td>
            <td>￥28,800円</td>
            <td>￥2,400円</td>
          </tr>
        </table>
        </div>
    </div>
  </section>
<?php include('footer.php'); ?>  