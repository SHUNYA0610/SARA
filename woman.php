<?php include('header4.php'); ?>
<main>
  <section class="sab-visual">
<?php include('nav.php'); ?>
    <div class="sab-visual-content">
      <h2 class="sab-visual-heading">Menu</h2>
    <div>
  </section>
  <div class="sabnav">
    <ul class="column2">
     <li><a href="index.php"><i class="fa-solid fa-house" style="color: #D085FD;"></i>トップ</a></li>
     <li><a href="menu.php"><i class="fa-solid fa-angle-up fa-rotate-90" style="color: #D085FD;"></i>メニュー</a></li>
     <li><a href="woman.php"><i class="fa-solid fa-angle-up fa-rotate-90" style="color: #D085FD;"></i>レディース脱毛</a></li>
    </ul>  
  </div>
  <section class=menu>
   <div class="tb-line">
      <h2>レディース脱毛 人気メニュー</h2>
   </div>    
   <div class=menu-contents>
     <div class="left-line">  
        <dl>
          <db>
            <div class="menu-heading">肩ワキ</div>
            <div class="menu-price">￥3,980円</div>
          </db>  
          <dt></dt>
        <dl>
          <db>        
            <div class="menu-heading">VIO(女性限定)</div>
            <div class="menu-price">￥9,980円</div>  
          </db>            
          <dt></dt>
        <dl>
          <db>        
            <div class="menu-heading">両足(両脚)</div>
            <div class="menu-price">￥5,980円</div>
          </db>            
          <dt></dt>
        <dl>
          <db>        
            <div class="menu-heading">両腕</div>
            <div class="menu-price">￥5,480円</div>
          </db>            
        </dl>
     </div>  
     <div class="menu-image"><img src="./img/woman.jpg"></div>
   </div>
   <div>
   <p class="menu-text">
      Contrary to popular belief, Lorem Ipsum is not simply random</br>
      text. It has roots in a piece of classical Latin literature from 45</br>
      one of the more obscure Latin words, consectetur</p>
   </div>    
  </section>
  <section class="price">
    <div class="price-content">
      <h2 id="price-heading">コース料金表</h2>
      <div class="table-scroll">
      <table class="price-table">
          <tr>
            <th class="fixed01">コース</th>
            <th>時間</th>
            <th>初来店<br>（お試し）</th>
            <th>１回（都度払い）</th>
            <th>６回コース</th>
            <th>１回分<br>[約5%オフ]</th>
        　　<th>１２回コース</th>
            <th>１回分<br>[約10%オフ]</th>
          </tr>
          <tr>
            <th class="fixed01">全身脱毛+(お顔+VIO)</th>
            <td>110分</td>
            <td>￥12,490円</td>
            <td>￥24,980円</td>
            <td>￥139,800円</td>
            <td>￥23,300円</td>
            <td>￥269,800円</td>
            <td>￥22,400円</td>
          </tr>
          <tr>
            <th class="fixed01" style="background-color: #fff;">全身脱毛+(VIO)</th>
            <td>90分</td>
            <td>￥10,990円</td>
            <td>￥21,980円</td>
            <td>￥124,800円</td>
            <td style="border-left: 1px #000 dashed">￥20,800円</td>
            <td>￥236,800円</td>
            <td>￥19,700円</td>
          </tr>
          <tr>
            <th class="fixed01">全身脱毛+(お顔)</th>
            <td>85分</td>
            <td>￥10,490円</td>
            <td>￥20,980円</td>
            <td>￥119,800円</td>
            <td>￥19,900円</td>
            <td>￥226,800円</td>
            <td>￥18,900円</td>
          </tr>
          <tr>
            <th class="fixed01" style="background-color: #fff;">全身脱毛</th>
            <td>60分</td>
            <td>￥8,990円</td>
            <td>￥17,980円</td>
            <td>￥102,800円</td>
            <td>￥17,100円</td>
            <td>￥193,800円</td>
            <td>￥16,100円</td>
          </tr>
          <tr>
            <th class="fixed01">VIO+両腕+両足</th>
            <td>50分</td>
            <td>￥6,490円</td>
            <td>￥14,980円</td>
            <td>￥7,800円</td>
            <td>￥12,300円</td>
            <td>￥139,800円</td>
            <td>￥11,600円</td>
          </tr>
          <tr>
            <th class="fixed01" style="background-color: #fff;">お顔+両腕+両足</th>
            <td>45分</td>
            <td>￥6,990円</td>
            <td>￥13,980円</td>
            <td>￥78,800円</td>
            <td>￥13,100円</td>
            <td>￥149,800円</td>
            <td>￥12,500円</td>
          </tr>
          <tr>
            <th class="fixed01">両腕+両ワキ+両足</th>
            <td>35分</td>
            <td>￥5,990円</td>
            <td>￥11,980円</td>
            <td>￥67,800円</td>
            <td>￥11,300円</td>
            <td>￥129,800円</td>
            <td>￥10,800円</td>
          </tr>
          <tr>
            <th class="fixed01" style="background-color: #fff;">VIO</th>
            <td>20分</td>
            <td>￥4,990円</td>
            <td>￥9,980円</td>
            <td>￥56,800円</td>
            <td>￥9,400円</td>
            <td>￥107,800円</td>
            <td>￥8,900円</td>
          </tr>
          <tr>
            <th class="fixed01">お顔</th>
            <td>20分</td>
            <td>￥4,490円</td>
            <td>￥8,980円</td>
            <td>￥50,800円</td>
            <td>￥8,400</td>
            <td>￥96,800円</td>
            <td>￥8,000円</td>
          </tr>
          <tr>
            <th class="fixed01" style="background-color: #fff;">うなじ・首・背中(上部)</th>
            <td>20分</td>
            <td>￥3,990円</td>
            <td>￥7,980円</td>
            <td>￥44,800円</td>
            <td>￥7,400円</td>
            <td>￥85,800円</td>
            <td>￥7,100円</td>
          </tr>
          <tr>
            <th class="fixed01">両腕<br>(ひじ・手の指・甲)</th>
            <td>15分</td>
            <td>￥2,740円</td>
            <td>￥5,480円</td>
            <td>￥30,800円</td>
            <td>￥5,100円</td>
            <td>￥58,800円</td>
            <td>￥4,900円</td>
          </tr>
          <tr>
            <th class="fixed01" style="background-color: #fff;">両足<br>(ひざ・足指・甲)</th>
            <td>20分</td>
            <td>￥2,990円</td>
            <td>￥5,980円</td>
            <td>￥33,800円</td>
            <td>￥5,600円</td>
            <td>￥63,800円</td>
            <td>￥5,300円</td>
          </tr>
          <tr>
            <th class="fixed01">肩ワキ</th>
            <td>10分</td>
            <td>￥1,990円</td>
            <td>￥3,980円</td>
            <td>￥22,800円</td>
            <td>￥3,800円</td>
            <td>￥42,800円</td>
            <td>￥3,500円</td>
          </tr>
        </table>
        </div>
    </div>
  </section>
<?php include('footer.php'); ?>  